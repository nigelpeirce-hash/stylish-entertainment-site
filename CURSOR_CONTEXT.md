...
- **Musicians indexable profile pages (logged May 21, 2026 — SEO audit item 6, deferred from Commit B):** `/artists/musicians/` is the only roster page without per-act profile URLs, so individual musicians can't rank or be linked to and the listing JSON-LD is page-level only (no `ItemList` with each act enumerated). Mirror the DJ infrastructure we shipped in commits `f7906d2` and `aa14db3`:
  1. New dynamic route `app/artists/musicians/[slug]/page.tsx` — server component, fetches by slug via Prisma `Musician` model (filter `isActive: true`); `generateMetadata` falls back `seoTitle` → `${name} | Live Wedding Musician`, `seoDescription` → `strapLine` → first 160 chars of `bio`. DB error → throw (5xx → Google retries), slug-not-found → `notFound()` → 404.
  2. JSON-LD: prefer `MusicGroup` for ensembles (quartet/trio/band) and `Person` for soloists (pianist, harpist, saxophonist). Reuse the OG image URL transform pattern from `app/artists/djs/[slug]/page.tsx` so `og:image` actually serves a 1200×630 Cloudinary variant (declared dimensions match payload).
  3. Convert `app/artists/musicians/page.tsx` to a server component (currently `"use client"` with `/api/musicians` fetch) — fetch musicians server-side via a new `lib/musician-data.ts#fetchActiveMusiciansForRoster()` Prisma helper, pass to a client island `MusiciansPageContent.tsx`. This lets the page's `CollectionPage` JSON-LD (currently in `layout.tsx`) gain a real `mainEntity.ItemList` enumerating each musician with `position`, `url`, `image`, `description`.
  4. Cross-link each card on the listing page to `/artists/musicians/[slug]/` (image + name + primary CTA), same pattern as the DJ cards in `DJsPageContent.tsx`.
  5. Add the musician slug URLs to `app/sitemap.ts` (dynamic Prisma query — guard against DB-unavailable at build time, same pattern as the DJ slug entries).
  6. Add explicit `PATH_SEGMENT_LABELS` entries in `lib/breadcrumb-config.ts` for each musician slug so breadcrumbs don't render auto-title-cased fallbacks (the DJ slugs already do this — see commit `aa14db3`).
  7. Consider whether musicians need a parallel `lib/musician-extras.ts` for residency / hero-image-URL / curated-testimonial overrides — only worth building if any musician has the same Babington-residency-style trust signal as DJ Nige; otherwise the DB fields are enough.
  - **Estimated effort:** 3–4 hours. No DB schema change needed (`Musician` model already has `slug`, `bio`, `fullBio`, `seoTitle`, `seoDescription`, `strapLine`, `imageUrl`, `isActive`). Smoke tests: each musician slug returns 200, OG image is 1200×630, sitemap includes the new URLs, `/artists/musicians/` ships `ItemList` with `numberOfItems` matching active musician count.
...
- **SEO audit follow-ups — Commit B (May 21, 2026):** Seven prioritised fixes from the live-site SEO audit, shipped together as one logical commit because they're all metadata/structured-data polish.
  ...
  3. **`/weddings/wedding-lighting/` title + description trimmed and Service JSON-LD added.** Title was 74 chars, description 178. Cut to 55 / 135 chars. Service JSON-LD with `areaServed` array (South West / London / Home Counties) and same `LocalBusiness` cross-link.
  4. **DJ profile hi-res OG images (all 4 DJs).** Profile `generateMetadata` was declaring `og:image:width=1200, og:image:height=630` while actually serving the 400×400 face-cropped Cloudinary thumbnail — social cards showed a tiny square. Added a one-line URL transform: takes `dj.imageUrl` and swaps its Cloudinary transformation segment to `f_auto,q_85,dpr_1,w_1200,h_630,c_fill,g_auto`. Works for every DJ without per-slug overrides; `g_auto` is content-aware so the social card isn't zoomed to the face.
  5. **CollectionPage + ItemList JSON-LD on `/artists/djs/`.** Server-rendered DJ listing now ships a `CollectionPage` containing an `ItemList` with each DJ as a `Person` (position, name, url, image, jobTitle, worksFor cross-link). Google can crawl the roster as a structured set rather than parsing card markup.
  6. **CollectionPage JSON-LD on `/artists/musicians/`.** Static page-level schema injected via the new `layout.tsx`. Individual musicians not enumerated (page is still client-rendered — that lands when we build `/artists/musicians/[slug]/` profile infrastructure, parallel to the DJ work).
  7. **Title polish: homepage and `/artists/djs/`.** Homepage was 29 chars ("Luxury DJs & Event Production"), now 40 chars ("Luxury Wedding DJs & Event Production UK") with description rewritten to mention Babington House residency since 2003. `/artists/djs/` was 30 chars ("DJs & Live DJ Acts UK-Wide"), now 41 chars ("Wedding DJs UK | Babington House Resident").

  Plus one supporting change to `lib/metadata.ts`: new optional `titleAbsolute?: boolean` flag that renders `title` as `{ absolute: ... }` to skip the root `title.template` ("%s | STYLISH Entertainment"). Used by `/weddings/wedding-lighting/` and `/weddings/wedding-entertainment/` because there's no `app/weddings/layout.tsx` between root and child — without `titleAbsolute` the rendered titles were 87 and 88 chars (both truncated in SERPs). Pattern documented in the JSDoc so future authors know when to opt in. **Outstanding:** musicians indexable profile pages (`/artists/musicians/[slug]/`) — a larger task that mirrors the DJ profile infrastructure (dynamic route + server-render + JSON-LD + sitemap), left for a separate session.
