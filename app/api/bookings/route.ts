...
export async function POST(request: NextRequest) {
  try {
    ...
    const trim = (s: string | undefined) => (s != null ? String(s).trim() || null : null);
    const booking = await prisma.booking.create({
      data: {
        ...
        clientAddress: trim(validatedData.clientAddress),
        clientAddress2: trim(validatedData.clientAddress2),
        clientTown: trim(validatedData.clientTown),
        clientCounty: trim(validatedData.clientCounty),
        clientPostcode: trim(validatedData.clientPostcode),
        eventType: validatedData.eventType,
        eventDate: eventDate,
        djStartTime: trim(validatedData.eventStartTime),
        djFinishTime: trim(validatedData.eventEndTime),
        venueName: venueName || "TBD",
        venueAddress: trim(validatedData.venueAddress),
        venueAddress2: trim(validatedData.venueAddress2),
        venueTown: trim(validatedData.venueTown),
        venueCounty: trim(validatedData.venueCounty),
        venuePostcode: trim(venuePostcode) || null,
        ...
        finalBalance: validatedData.agreedFee && trim(validatedData.agreedFee) ? trim(validatedData.agreedFee) : null,
