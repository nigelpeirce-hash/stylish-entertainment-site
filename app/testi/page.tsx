...
export default function Testimonials() {
  // Use state to shuffle testimonials only on client after hydration
  const [shuffledTestimonials, setShuffledTestimonials] = useState(testimonials);

  useEffect(() => {
    // Shuffle after component mounts on client to avoid hydration mismatch
    setShuffledTestimonials(shuffleArray(testimonials));
    
    document.title = "Testimonials | Client Reviews | Stylish Entertainment";
    const metaDescription = document.querySelector('meta[name="description"]');
    if (metaDescription) {
      metaDescription.setAttribute("content", "Read testimonials from our happy clients. Real wedding and party reviews from venues including Babington House, Pencarrow Estate, Northover Manor, and Rockingham Castle.");
    }
  }, []);
  ...
  return (
    <div>
      ...
      <section className="relative min-h-[60vh] flex items-center justify-center bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white overflow-hidden">
        ...
        >
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-sans mb-4 sm:mb-6 text-white font-bold px-4 drop-shadow-lg">Client Testimonials</h1>
              ...
              >
                <Card className="bg-gray-900 border-champagne-gold/30 h-full">
                  <CardContent className="p-6 sm:p-8">
                    <p className="text-gray-300 mb-4 leading-relaxed italic text-base sm:text-lg">
                      &quot;{testimonial.quote}&quot;
                    </p>
                      ...
                      <p className="text-gray-400 text-xs sm:text-sm mt-1">
                        {testimonial.venueUrl ? (
                          <Link 
                            ...
                            className="hover:text-champagne-gold transition-colors underline"
                          >
                            {testimonial.venue}
                          </Link>
                        ) : (
